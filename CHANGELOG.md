...
## 2026-04-09 18:19:35 IST
- Made DFS-lite replication behavior explicit in the master runtime and dashboard by exposing requested versus effective replication, available worker count, and a warning when the configured replication factor exceeds live worker capacity. This prevents silent `replication_factor` downscaling from looking like successful triple replication when only one worker is registered. Files: `master/master_dfs.py`, `master/templates/index_dfs.html`, `tests/test_dfs_lite_workflow.py`, `tests/test_dashboard_ui.py`, `CHANGELOG.md`
- Added verification coverage for both the backend and browser layers: a runtime snapshot test now asserts the effective replication factor is capped when worker capacity is insufficient, a UI test checks the warning banner, and a new three-worker integration test proves that factor-3 replication commits every block into three distinct worker storage directories when three workers actually exist. Files: `tests/test_dfs_lite_workflow.py`, `tests/test_dashboard_ui.py`, `CHANGELOG.md`

## 2026-04-09 18:27:17 IST
- Hardened manual training start so the master now refuses to launch a run when workers are registered but unreachable, instead of proceeding into a misleading block-commit failure. The control plane now refreshes worker health before starting and emits a clear error instructing the operator to bring a worker online or fix its advertised endpoint. Added a regression test covering the offline-worker start path. Files: `master/master_dfs.py`, `tests/test_dfs_lite_workflow.py`, `CHANGELOG.md`

## 2026-04-09 18:32:42 IST
- Corrected the worker dashboard connection semantics so it no longer treats a one-time successful registration POST as proof that the master can still reach the worker. Worker telemetry now queries a cache-only master observation route and exposes the master’s current view of that worker, including the endpoint the master sees and an explicit offline error when the master marks the worker unhealthy, without creating recursive master-worker status polling. The worker dashboard banner and error panel now reflect that master-side observation instead of a stale “Connected” message, while still staying neutral until the master has recorded a live health snapshot. Added backend and browser regression coverage for the offline-observed state. Files: `master/master_dfs.py`, `worker/worker_dfs.py`, `worker/templates/index_dfs.html`, `tests/test_dfs_lite_workflow.py`, `tests/test_dashboard_ui.py`, `CHANGELOG.md`

## 2026-04-09 18:56:15 IST
- Fixed DFS-lite block placement so training uses only the currently healthy worker subset at run start instead of blindly partitioning across every registered worker. This prevents a single offline worker from receiving a shard assignment and crashing the run with `Block ... could not be committed to any worker` when another worker is still healthy and available. Added regression coverage for the mixed healthy/offline cluster path while preserving the existing multi-worker and factor-3 replication behavior. Files: `master/master_dfs.py`, `tests/test_dfs_lite_workflow.py`, `CHANGELOG.md`

## 2026-04-09 19:02:24 IST
- Hardened Windows worker endpoint discovery so candidate addresses now also include IPv4s parsed from native OS network command output instead of relying only on hostname resolution and outbound socket probes. This gives the master more realistic endpoint candidates to probe when Windows hosts expose multiple adapters and reduces false offline flags caused by selecting an unreachable virtual-interface address. Added parser coverage for Windows- and POSIX-style interface output while excluding broadcast, netmask, loopback, and link-local addresses. Files: `worker/worker_dfs.py`, `tests/test_dfs_lite_workflow.py`, `CHANGELOG.md`

## 2026-04-10 13:00:16 IST
- Hardened master-side worker endpoint recovery by persisting each worker's endpoint candidate list in runtime config and using those stored candidates during health refresh. If the currently selected endpoint becomes unreachable but another stored candidate responds to `/health`, the master now rewrites the worker endpoint to that reachable candidate automatically instead of keeping the worker falsely offline indefinitely. Added regression coverage for candidate persistence and health-refresh endpoint failover. Files: `master/master_dfs.py`, `tests/test_dfs_lite_workflow.py`, `CHANGELOG.md`
...
- 2026-04-09 17:10:00 IST: Hardened the zero-argument startup path so fresh masters and workers can converge without manual worker lists or endpoint flags. The default DFS-lite config now starts with no preloaded localhost workers, the master advertises its own HTTP endpoint over UDP, workers listen for master beacons and self-register automatically, and master autostart now waits for healthy workers instead of failing immediately on an empty cluster. Added integration coverage for worker-side master discovery and delayed autostart registration. Files: `master/master_dfs.py`, `worker/worker_dfs.py`, `config_extended.json`, `tests/test_dfs_lite_workflow.py`, `README.md`, `CHANGELOG.md`
- 2026-04-09 17:50:00 IST: Added a larger builtin dataset option across both master runtimes by supporting `digits` alongside `breast_cancer`. The baseline and DFS-lite masters now load `digits` through the same builtin dataset path, the DFS runtime config validator accepts it, and regression tests cover builtin selection in both flows. Files: `master/master.py`, `master/master_dfs.py`, `tests/test_federated_workflow.py`, `tests/test_dfs_lite_workflow.py`, `README.md`, `CHANGELOG.md`
